...
func TestNextDNSDenylistReconciler_countActiveDomains(t *testing.T) {
	tests := []struct {
		name     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			reconciler := &NextDNSDenylistReconciler{}
...
func TestNextDNSDenylistReconciler_findProfileReferences(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = nextdnsv1alpha1.AddToScheme(scheme)

	tests := []struct {
		name     string
		list     *nextdnsv1alpha1.NextDNSDenylist
...
func TestNextDNSDenylistReconciler_Reconcile(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = nextdnsv1alpha1.AddToScheme(scheme)
	...
	fakeClient := fake.NewClientBuilder().
		...
		WithStatusSubresource(&nextdnsv1alpha1.NextDNSDenylist{}).
		Build()
	...
	// Get updated list
	var updatedList nextdnsv1alpha1.NextDNSDenylist
	err = fakeClient.Get(context.Background(), req.NamespacedName, &updatedList)
	assert.NoError(t, err)
	assert.Contains(t, updatedList.Finalizers, DenylistFinalizerName)
...
func TestNextDNSDenylistReconciler_HandleDeletion(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = nextdnsv1alpha1.AddToScheme(scheme)
	...
	t.Run("deletion blocked when profiles reference list", func(t *testing.T) {
		...
		list := &nextdnsv1alpha1.NextDNSDenylist{
			ObjectMeta: metav1.ObjectMeta{
				Name:              "test-list",
				Namespace:         "default",
				Finalizers:        []string{DenylistFinalizerName},
			...
			Status: nextdnsv1alpha1.NextDNSDenylistStatus{
...
func TestNextDNSDenylistReconciler_findDenylistsForProfile(t *testing.T) {
	scheme := runtime.NewScheme()
	_ = nextdnsv1alpha1.AddToScheme(scheme)

	list1 := &nextdnsv1alpha1.NextDNSDenylist{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "list1",
			Namespace: "default",
		},
	...
	list2 := &nextdnsv1alpha1.NextDNSDenylist{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "list2",
			Namespace: "other",
		},
	...
	requests := r.findDenylistsForProfile(context.Background(), profile)
